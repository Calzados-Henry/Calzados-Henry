...
const DbModelsAdd = require('../../db.ts')
...
export const getAddress = async (id: string): Promise<object> => {
    const userAddresses: any = await DbModelsAdd.Users.findByPk(id, { include: { model: DbModelsAdd.Address } })
...
export const postAddress = async (id: string, body: any): Promise<object> => {
    ...
    const [newAddress, created] = await DbModelsAdd.Address.findOrCreate({
        where: {
            address: address,
            zip_code: zip_code
        }
    })
    const user = await DbModelsAdd.Users.findByPk(id)
    if (user && created) {
        await user.addAddress(newAddress)
        const userAddresses = await DbModelsAdd.Users.findByPk(id, { include: { model: DbModelsAdd.Address } })
        if (userAddresses) {
...
export const patchAddress = async (value: any): Promise<object> => {
    const address = await DbModelsAdd.Address.findByPk(value.id)
...
export const deleteAddress = async (value: any): Promise<object> => {
    const deletedAddress = await DbModelsAdd.Address.findByPk(value.id)
