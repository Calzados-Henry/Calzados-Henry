...
export const createSize = async (value: any): Promise<any> => {
  ...
  var size: any = await Size.findAll({ where: { size: String(value.size) } })
  if (size.length > 0) {
...
export const updateSize = async (value: any): Promise<any> => {
  ...
  var sizeByID = await Size.findByPk(value.id)
  var sizeDuplicate = await Size.findAll({ where: { size: String(value.size) } })
  if (sizeByID !== null) {
...
export const deleteSize = async (id: number): Promise<any> => {
  ...
  var sizeByID: any = await Size.findByPk(id)
  if (sizeByID !== null) {
